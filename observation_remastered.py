...
def get_block_coordinates(i, grid_size, adjustment_to_center):
    x = i % grid_size
    y = i // grid_size // grid_size
    z = (i // grid_size) % grid_size
    return (x - adjustment_to_center), (y + 227), (z - adjustment_to_center)

def get_block1d_coordinates(x, y, z, grid_size, adjustment_to_center):
    # do the inverse of getBlockCoordinates
    return (x + adjustment_to_center) + (y - 227) * grid_size * grid_size + (z + adjustment_to_center) * grid_size

def compare_block(block_origin, block_hit):
    precision = 1.49
    if abs(block_origin.x - block_hit.x) > precision or abs(block_origin.y - block_hit.y) > precision or abs(block_origin.z - block_hit.z) > precision:
        return False
    # compare the type of the block
    if block_origin.type != block_hit.type:
        return False
    return True

def update_grid(observation, grid, side_size, grid_types):
    ...
    if "floor" in observation:
        ...
        adjustment_to_center = (side_size - 1) // 2
        ...
        if u"LineOfSight" in observation:
            ...
            change = grid_check(grid, los, floor, side_size, adjustment_to_center, grid_types)
        # check all blocks we are not looking at are in grid are in the observation  
        change = check_grid_integrity(grid, floor, side_size, adjustment_to_center, grid_types) or change
    return change

def grid_check(grid, los, floor, grid_size, adjustment_to_center, grid_types):
    ...
    index = [int(get_block1d_coordinates(x, y, z, grid_size, adjustment_to_center)) for x, y, z in cords]
    for i in index:
        ...
        if i < len(floor):
            ...
            x, y, z = get_block_coordinates(i, grid_size, adjustment_to_center)
...
def check_grid_integrity(grid, floor, grid_size, adjustment_to_center, grid_types):
    # check if all grids are in the floor
    for key in grid:
        block = grid[key]
        # get the index of the block in the floor
        index = get_block1d_coordinates(block.x, block.y, block.z, grid_size, adjustment_to_center)
...
#             if block == None:
#                 block = grid[b]
#             # check which block is closer to the cord
